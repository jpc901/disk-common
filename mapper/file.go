...
func UpdateFile(fileHash string, fileName string, fileSize int64, fileAddr string) error {
	...
	stmt, err := db.GetDBInstance().GetConn().Prepare("insert ignore into tbl_file" +
...
func GetFileMeta(fileHash string) (*TableFile, error) {
	stmt, err := db.GetDBInstance().GetConn().Prepare("select file_sha1,file_addr,file_name,file_size from tbl_file where file_sha1=? and status=1 limit 1")
	if err != nil {
		log.Error(err.Error())
		return nil, err
	}
...
func DeleteUserFile(fileHash, fileName string) error {
	stmt, err := db.GetDBInstance().GetConn().Prepare("delete from tbl_user_file where file_sha1=? and file_name=? limit 1")
	if err != nil {
		log.Error(err.Error())
		return err
	}
